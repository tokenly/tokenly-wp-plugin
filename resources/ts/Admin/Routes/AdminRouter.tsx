...
import TokenMetaFeaturedImageColumn
	from '../Pages/Listings/Token/MetaFeaturedImageColumn'
import UserCreditBalanceColumn
	from '../Pages/Listings/User/UserCreditBalanceColumn'
...
export default class AdminRouter implements AdminRouterInterface {
	...
	protected getViews(): object {
		let routes = {
			...
			[`column-${this.namespace}-${this.namespace}_token_meta-featured-image`]
			                               : TokenMetaFeaturedImageColumn,
	...
	protected getHighlights(): any {
		const adminBaseUrl = '/wp-admin/admin.php?page='
		const highlights = {
			[`${this.namespace}-token-vendor`]: [
				`${adminBaseUrl}${this.namespace}-token-address-index`,
				`${adminBaseUrl}${this.namespace}-token-address-show`,
				`${adminBaseUrl}${this.namespace}-token-address-store`,
				`${adminBaseUrl}${this.namespace}-token-address-edit`,
				`${adminBaseUrl}${this.namespace}-token-address-verify`,
				`${adminBaseUrl}${this.namespace}-token-address-balance-index`,
				`${adminBaseUrl}${this.namespace}-token-promise-index`,
				`${adminBaseUrl}${this.namespace}-token-promise-show`,
				`${adminBaseUrl}${this.namespace}-token-promise-store`,
				`${adminBaseUrl}${this.namespace}-token-promise-edit`,
				`${adminBaseUrl}${this.namespace}-token-source-index`,
				`${adminBaseUrl}${this.namespace}-token-source-show`,
				`${adminBaseUrl}${this.namespace}-token-source-store`,
				`${adminBaseUrl}${this.namespace}-token-source-edit`,
				`${adminBaseUrl}${this.namespace}-token-meta-edit`,
				`${adminBaseUrl}${this.namespace}-token-vendor-show`,
				`${adminBaseUrl}${this.namespace}-token-whitelist-edit`,
				`${adminBaseUrl}${this.namespace}-user-token-balance-index`,
				`/wp-admin/edit.php?post_type=${this.namespace}_token_meta`,
				`/wp-admin/edit-tags.php?taxonomy=${this.namespace}
				_token_category&post_type=${this.namespace}_token_meta`,
			],
			[ `${this.namespace}-credit-vendor` ]: [
				`${adminBaseUrl}${this.namespace}-credit-group-account-index`,
				`${adminBaseUrl}${this.namespace}-credit-group-index`,
				`${adminBaseUrl}${this.namespace}-credit-group-show`,
				`${adminBaseUrl}${this.namespace}-credit-group-store`,
				`${adminBaseUrl}${this.namespace}-credit-group-edit`,
				`${adminBaseUrl}${this.namespace}-credit-group-whitelist-edit`,
				`${adminBaseUrl}${this.namespace}-credit-transaction-index`,
				`${adminBaseUrl}${this.namespace}-credit-transaction-store`,
				`${adminBaseUrl}${this.namespace}-user-credit-balance-index`,
			],
		} as any
		return highlights
	}
	 ...
	 */
	protected render(
		element: HTMLElement,ViewComponent: any, props: any, view: string
	): void {
	...
	protected highlightMenu(): void {
		const adminMenu = document.querySelector(
			`#adminmenu #toplevel_page_${this.namespace}`
		)
		if ( !adminMenu ) {
			return
		}
