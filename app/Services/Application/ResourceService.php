...
class ResourceService extends Service implements ResourceServiceInterface {
	...
	public function register(): void {
		...
		add_filter(
			"{$this->namespace}_shared_data",
			array( $this, 'append_shared_data' )
		);
	}
	...
	public function enqueue_frontend_scripts(): void {
		...
		wp_register_script($label,"{$this->build_url}/Frontend.js",
	...
	public function enqueue_admin_scripts(): void {
		...
		wp_register_script( $label, "{$this->build_url}/Admin.js",
			array( 'wp-components', 'wp-element' ), $version, true
		);
		wp_enqueue_script( $label );
		wp_register_style( $label, "{$this->build_url}/Admin.css",
			array( 'wp-components' ), $version
		);
		wp_enqueue_style( $label );
		$this->enqueue_shared_script();
	}
	...
	public function append_shared_data( array $shared_data = array() ): array {
		...
		$shared_data = array_merge( $shared_data, array(
			...
			'nonce'                   => $nonce,
			'integration_can_connect' => $can_connect,
		...
		if ( $user ) {
			...
			$shared_data['user_can_connect'] = $user->can_connect;
	...
	protected function enqueue_shared_script(): void {
		...
		wp_register_script( "{$this->namespace}_shared_data", false, array(),
			false, false );
