  ...
  if(isset($access_token)){
    ...
    if ( $user_id != 'false') {
      if ( ! $user_id && false == email_exists( $user_email ) ) {
        ...
        $redirect_url = $current_web_url.'/wp-admin/?error=no&user_register=yes&useremail='.$username.'';
        ...
        if ( is_wp_error( $user ) ) {
          ...
          $redirect_url = $current_web_url.'/wp-admin/?error=yes&logged_in=no&useremail='.$username.'&message='.$signin_message.'';
        }else{
          add_user_meta( $user->data->ID, 'access-token', $access_token);
          $signin_message =  'logged in';
          $redirect_url = $current_web_url.'/wp-admin/?error=no&logged_in=yes&useremail='.$username.'&message='.$signin_message.'';
        }
        echo "<script>window.location.href='".$redirect_url."';</script>";
      ...
      $redirect_url = $current_web_url.'/wp-admin/?error=yes&message=No,user found with this username '.$username.'';
      echo "<script>window.location.href='".$redirect_url."';</script>";
    
    }  
  }else{
    echo "<script> alert( 'Access Token error!' );</script>"; 
  }
}else{
