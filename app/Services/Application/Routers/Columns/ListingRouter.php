...
class ListingRouter extends Router implements ListingRouterInterface {
	...
	public function register_actions( callable $new_actions ): void {
		add_action( 'admin_init', function() use ( $new_actions ) {
			add_filter( "{$this->namespace}_{$this->action_type}_row_actions", function( array $actions, object $target ) use ( $new_actions ) {
	 ...
	 * @inheritDoc
	 */
	public function register_column( string $key, ColumnInterface $column, string $type ): void {
		$callback_render = $this->get_column_render_callback( $key, $column );
		$callback_register = $this->get_column_register_callback( $key, $column );
		add_filter( "manage_{$type}_columns", $callback_register );
		add_action( "manage_{$type}_custom_column", $callback_render, 10, 3 );
	}

	protected function get_column_render_callback( $key, ColumnInterface $column ): callable {
		return function(){};
	}

	protected function get_column_register_callback( $key, ColumnInterface $column ): callable {
		return function(){};
	}
}
